  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <Link href="/facility/staff" className="btn-secondary text-sm">
          Team members
        </Link>
      </div>
      <div className="mb-4 flex flex-wrap gap-2">
        <SearchBar value={q} onChange={(v) => { setQ(v); setPage(1); }} placeholder="Search doctorsΓÇª" />
        ...
        <div className="mb-4 space-y-4">
          <div className="card space-y-3">
            ...
            >
              ...
              >
                {lookup.isPending ? 'SearchingΓÇª' : 'Find in register'}
              </button>
            </form>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-teal-200 bg-teal-50 p-3 dark:border-teal-800 dark:bg-teal-950">
                <div>
                  ...
                  <p className="text-sm text-[var(--muted)]">
                    ...
                    {lookupResult.provider.mdpczNumber ?? 'ΓÇö'}
                  </p>
                </div>
                {lookupResult.provider.alreadyAtFacility ? (
                  ...
                  >
                    {attach.isPending ? 'AttachingΓÇª' : 'Attach to this facility'}
                  </button>
                )}
              </div>
            )}
                ...
                <SectionCard key={String(d.id)} title={String(d.name)} className="mb-0">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm">MDPCZ: {String(d.mdpczNumber ?? 'ΓÇö')}</p>
                      <p className="text-sm">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            <button
              ...
              onClick={() => save.mutate()}
            >
              {save.isPending ? 'SavingΓÇª' : 'Save hours'}
            </button>
            <button type="button" className="btn-secondary text-sm" onClick={onClose}>
              Close
            </button>
