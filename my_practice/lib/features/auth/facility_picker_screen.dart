...
class _FacilityPickerScreenState extends ConsumerState<FacilityPickerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final claimable =
        linked.where((f) => f.canClaimOwnership || f.isOwnedByMe).toList();
      ...
      body: _refreshing
          ...
          : ListView(
              ...
              children: [
                ...
                if (memberships.isNotEmpty) ...[
                  ...
                  Text(
                    memberships.length > 1
                        ? 'Tap a facility to switch your working site in MyPractice.'
                        : 'Open your facility or claim additional registry-linked sites below.',
                    style: AppTextStyles.sm(
                      color: context.appColors.mutedForeground,
                    ),
                  ),
                ...
                ...buildClaimableFacilitySection(
                  context,
                  claimable: claimable,
                  claimingId: _claimingId,
                  error: _error,
                  onClaim: _claimAndOpen,
                  onOpenOwned: _openOwned,
                  sectionTitle: memberships.isEmpty
                      ? 'Registry-linked facilities'
                      : 'More linked facilities',
                ),
                if (memberships.isEmpty && claimable.isEmpty)
                  Padding(
                    padding: const EdgeInsets.symmetric(vertical: 24),
                    child: Text(
                      auth.error ?? 'No facilities linked to your account yet.',
                ...
                OutlinedButton.icon(
                  ...
                  label: const Text('More claim options'),
                ),
