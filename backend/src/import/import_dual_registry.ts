...
Options:
  ...
  --skip-geocoding  Skip Nominatim (cache-only geocoding for facilities)
`);
    process.exit(1);
  }
    ...
    if (!opts.dryRun) {
      await client.query(
        ...
        [
          ...
          opts.dryRun,
          JSON.stringify({ reset: opts.reset, skipGeocoding: opts.skipGeocoding }),
        ],
      );
    }

    const hpaResult = await importHpaFacilities(
      client,
      opts.hpaFile,
      batchId,
      opts.dryRun,
