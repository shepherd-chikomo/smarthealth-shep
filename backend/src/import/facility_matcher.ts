...
import { resolveFacilityType } from './infer_facility_type.js';
...
function collectProviderSpecialtySignals(provider: NormalizedProvider): string[] {
  const signals: string[] = [];
  if (provider.specialtySlug) signals.push(provider.specialtySlug);
  if (provider.specialtyNormalized) signals.push(provider.specialtyNormalized);
  return signals;
}
...
export function extractFacilitiesFromProviders(
  ...
  const specialtyByKey = new Map<string, string[]>();
  ...
  for (const provider of providers) {
    ...
    const signals = collectProviderSpecialtySignals(provider);
    const existingSignals = specialtyByKey.get(key) ?? [];
    specialtyByKey.set(key, [...existingSignals, ...signals]);
  ...
  for (const [key, facility] of facilityMap) {
    facility.facilityType = resolveFacilityType({
      name: facility.name,
      facilityCategory: facility.facilityCategory,
      practiceType: null,
      specialtyTexts: specialtyByKey.get(key) ?? [],
    });
  }
